...
define('FP_PERFORMANCE_VERSION', '1.5.2');
...
class FP_Performance_Suite {
    ...
    private function init_components() {
        ...
        if (!class_exists('FP_Cache_Manager')) {
            $this->load_class('class-cache-manager.php');
        }
        
        if (!class_exists('FP_Database_Optimizer')) {
            $this->load_class('class-database-optimizer.php');
        }
        
        if (!class_exists('FP_Asset_Optimizer')) {
            $this->load_class('class-asset-optimizer.php');
        }
        
        if (!class_exists('FP_Mobile_Optimizer')) {
            $this->load_class('class-mobile-optimizer.php');
        }
        
        if (!class_exists('FP_Performance_Monitor')) {
            $this->load_class('class-performance-monitor.php');
        }
    }
    
    /**
     * Carica una classe se il file esiste
     */
    private function load_class($filename) {
        $file_path = FP_PERFORMANCE_PLUGIN_DIR . 'includes/' . $filename;
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            'fp-performance',
            __('Security', 'fp-performance'),
            __('Security', 'fp-performance'),
            'manage_options',
            'fp-performance-security',
            [$this, 'security_page']
        );
    ...
    private function register_settings() {
        ...
        add_settings_field(
            ...
            __('Database Optimization', 'fp-performance'),
            [$this, 'database_optimization_callback'],
            'fp-performance',
            'fp_performance_general'
        );
        ...
        add_settings_field(
            'asset_optimization',
            __('Asset Optimization', 'fp-performance'),
        ...
        add_settings_field(
            'mobile_optimization',
            __('Mobile Optimization', 'fp-performance'),
            [$this, 'mobile_optimization_callback'],
            'fp-performance',
            'fp_performance_general'
        );
    }
        ...
        <div class="wrap">
            ...
            <div class="fp-performance-dashboard">
                ...
                <div class="fp-performance-stats">
                    ...
                    <div class="fp-performance-metrics">
                        <div class="metric">
                            ...
                            <span class="metric-value"><?php echo isset($options['cache_enabled']) && $options['cache_enabled'] ? __('Active', 'fp-performance') : __('Inactive', 'fp-performance'); ?></span>
                        </div>
                        <div class="metric">
                            <span class="metric-label"><?php _e('Database Status', 'fp-performance'); ?></span>
                            <span class="metric-value"><?php echo isset($options['database_optimization']) && $options['database_optimization'] ? __('Optimized', 'fp-performance') : __('Not Optimized', 'fp-performance'); ?></span>
                        </div>
                        <div class="metric">
                            <span class="metric-label"><?php _e('Assets Status', 'fp-performance'); ?></span>
                            <span class="metric-value"><?php echo isset($options['asset_optimization']) && $options['asset_optimization'] ? __('Optimized', 'fp-performance') : __('Not Optimized', 'fp-performance'); ?></span>
                        </div>
                        <div class="metric">
                            <span class="metric-label"><?php _e('Mobile Status', 'fp-performance'); ?></span>
                            <span class="metric-value"><?php echo isset($options['mobile_optimization']) && $options['mobile_optimization'] ? __('Optimized', 'fp-performance') : __('Not Optimized', 'fp-performance'); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="fp-performance-actions">
                    ...
                    <button type="button" class="button button-secondary" id="fp-optimize-mobile"><?php _e('Optimize Mobile', 'fp-performance'); ?></button>
                </div>
            </div>
        </div>
        <?php
    ...
    public function database_page() {
        ...
        <div class="wrap">
            ...
            <form method="post">
                <input type="hidden" name="action" value="fp_optimize_database">
                <?php wp_nonce_field('fp_optimize_database', 'fp_optimize_database_nonce'); ?>
                <button type="submit" class="button button-primary"><?php _e('Optimize Database', 'fp-performance'); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function assets_page() {
        ...
        <div class="wrap">
            ...
            <form method="post">
                <input type="hidden" name="action" value="fp_optimize_assets">
                <?php wp_nonce_field('fp_optimize_assets', 'fp_optimize_assets_nonce'); ?>
                <button type="submit" class="button button-primary"><?php _e('Optimize Assets', 'fp-performance'); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function mobile_page() {
        ...
        <div class="wrap">
            ...
            <form method="post">
                <input type="hidden" name="action" value="fp_optimize_mobile">
                <?php wp_nonce_field('fp_optimize_mobile', 'fp_optimize_mobile_nonce'); ?>
                <button type="submit" class="button button-primary"><?php _e('Optimize Mobile', 'fp-performance'); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function security_page() {
        ?>
        <div class="wrap">
            <h1><?php _e('Security Settings', 'fp-performance'); ?></h1>
            <p><?php _e('Configure security settings for your site.', 'fp-performance'); ?></p>
        </div>
        <?php
    }
     ...
     * Test mobile
     */
    private function test_mobile() {
        // Implementa test mobile
        return true;
    }
    ...
    public function sanitize_options($options) {
        ...
        if (isset($options['database_optimization'])) {
            $sanitized['database_optimization'] = (bool) $options['database_optimization'];
        }
        
        if (isset($options['asset_optimization'])) {
            $sanitized['asset_optimization'] = (bool) $options['asset_optimization'];
        }
        
        if (isset($options['mobile_optimization'])) {
            $sanitized['mobile_optimization'] = (bool) $options['mobile_optimization'];
        }
    ...
    /**
     * Callback per database optimization
     */
    public function database_optimization_callback() {
        $options = get_option('fp_performance_options', []);
        $checked = isset($options['database_optimization']) ? $options['database_optimization'] : false;
        ?>
        <input type="checkbox" name="fp_performance_options[database_optimization]" value="1" <?php checked($checked); ?> />
        <?php
    }
    
    /**
     * Callback per asset optimization
     */
    public function asset_optimization_callback() {
        $options = get_option('fp_performance_options', []);
        $checked = isset($options['asset_optimization']) ? $options['asset_optimization'] : false;
        ?>
        <input type="checkbox" name="fp_performance_options[asset_optimization]" value="1" <?php checked($checked); ?> />
        <?php
    }
    
    /**
     * Callback per mobile optimization
     */
    public function mobile_optimization_callback() {
        $options = get_option('fp_performance_options', []);
        $checked = isset($options['mobile_optimization']) ? $options['mobile_optimization'] : false;
        ?>
        <input type="checkbox" name="fp_performance_options[mobile_optimization]" value="1" <?php checked($checked); ?> />
        <?php
    }
}
